...
import { X, Cpu, Zap, Activity, ShieldCheck, Lock, Layers } from 'lucide-react';
...
export default function ReasoningGroqModal() {
  ...
  return (
    <>
      ...
      >
        <span className="text-slate-500 group-hover:text-cyan-500/70 transition-colors">Reasoning</span>
        <span className="text-slate-300 group-hover:text-cyan-400 transition-colors">Groq</span>
      </button>
                      ...
                      {engineSpecs.map((spec, idx) => {
                        ...
                        return (
                          <motion.div
                            ...
                            className={`flex flex-col gap-2.5 p-4 rounded-xl border ${spec.border} ${spec.bg} ${spec.hoverBorder} ${spec.hoverShadow} bg-gradient-to-b from-white/[0.04] to-transparent backdrop-blur-md transition-all duration-300 hover:scale-[1.01] group`}
                          >
