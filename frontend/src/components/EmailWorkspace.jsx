...
export default function EmailWorkspace({ setReportData, setIsInvestigating, targetLanguage = 'English' }) {
  ...
  return (
    <div className="w-full">
      {/* File Upload Zone */}
      <div 
        className={`w-full border-2 border-dashed border-cyan-500/20 hover:border-cyan-500/40 bg-zinc-950/70 backdrop-blur-md shadow-xl shadow-cyan-950/30 rounded-lg md:rounded-xl p-3.5 sm:p-5 md:p-8 flex flex-col items-center justify-center transition-colors duration-300 ${!isSubmitting ? 'cursor-pointer' : 'opacity-50 cursor-not-allowed'}`}
      ...
      >
        {emailFile ? (
          <div className="text-cyan-400 font-mono text-center flex flex-col items-center">
            <p className="text-sm sm:text-base md:text-lg font-bold">File Selected</p>
            <p className="text-xs sm:text-sm mt-1 md:mt-2 truncate max-w-[200px]">{emailFile.name}</p>
          </div>
        ) : (
          <div className="text-center text-slate-400 flex flex-col items-center">
            <UploadCloud className="text-cyan-500 w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 mb-1.5 md:mb-3" />
            <span className="text-slate-300 font-medium text-xs sm:text-sm md:text-base block">Submit Original Email Artifact</span>
            <span className="text-[10px] sm:text-xs font-mono text-slate-500 mt-1 md:mt-2 block">
              [ Drag & Drop or Click to Browse ]
            </span>
            <span className="text-[9px] sm:text-[10px] text-slate-500 mt-0.5 md:mt-1 font-mono block">Supports: .eml, .msg</span>
          </div>
        )}
      ...
      <div className="flex flex-wrap justify-center gap-2 sm:gap-4 mt-2.5 md:mt-4 text-[9px] sm:text-xs font-mono text-cyan-500/70">
      ...
      {/* The Divider */}
      <div className="flex items-center w-full my-2.5 sm:my-4 md:my-6 opacity-50">
        <div className="flex-grow border-t border-slate-800"></div>
        <span className="px-3 md:px-4 text-[9px] sm:text-[10px] font-mono text-slate-600 tracking-widest">OR</span>
        <div className="flex-grow border-t border-slate-800"></div>
      </div>

      {/* Text Area */}
      <textarea
        className={`w-full h-24 sm:h-28 md:h-40 bg-zinc-950/70 backdrop-blur-md border border-cyan-500/20 hover:border-cyan-500/40 transition-colors duration-300 shadow-xl shadow-cyan-950/30 rounded-lg md:rounded-xl p-2.5 md:p-4 text-zinc-300 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 resize-none font-sans text-xs md:text-sm leading-relaxed placeholder-slate-600 ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
      ...
      <div className="flex items-start mt-2 md:mt-3 space-x-2 text-slate-400 text-[10px] sm:text-xs font-mono">
      ...
      {/* The Action Button */}
      <button
        className={`w-full py-2.5 sm:py-3.5 md:py-4 mt-3 sm:mt-5 md:mt-8 font-mono tracking-widest text-xs sm:text-sm md:text-base rounded-lg md:rounded-xl transition-all duration-300 ${(!emailFile && !emailText.trim()) || isSubmitting ? 'bg-slate-900/50 border border-slate-800 text-slate-600 cursor-not-allowed' : 'bg-cyan-950/40 border border-cyan-500/50 text-cyan-400 hover:bg-cyan-900/60 hover:shadow-[0_0_15px_rgba(6,182,212,0.2)]'}`}
