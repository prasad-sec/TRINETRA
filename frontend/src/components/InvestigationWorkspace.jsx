...
const InvestigationWorkspace = ({ onStateChange, isDashboardActive = true }) => {
  ...
  const handleStartInvestigation = async () => {
    ...
    if (investigationState === 'idle') {
      ...
      setActiveStage(0); // Artifact Received
  ...
  return (
    <div className={`flex-1 flex flex-col items-center justify-start relative p-4 md:p-8 transition-colors duration-500 w-full overflow-hidden`}>
      ...
      <div className="flex-1 w-full max-w-5xl flex flex-col md:flex-row items-center justify-center gap-16 md:gap-32 z-10">
        ...
        >
          ...
          />
          {isDashboardActive && (
            <AIAssistantEye state={isInvestigating ? 'investigating' : 'idle'} className="w-full h-full" />
          )}
        </motion.div>
            ...
            {isInvestigating && (
              ...
              >
                <div className="mb-4 md:mb-6">
                  <h3 className="font-sans text-xs md:text-sm font-semibold tracking-widest text-cyan-400 uppercase mb-1">Investigation Active</h3>
                  <p className="font-mono text-[10px] md:text-xs text-slate-500">Analyzing artifact signatures and behavior...</p>
                </div>
