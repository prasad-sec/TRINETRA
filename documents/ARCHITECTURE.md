...
4. **QR Code Quishing Decoder (`/api/investigate/qr`):**
   ...
   - **Feature Extraction:** Stage 1 applies local image resizing, thresholding, and **bitwise-NOT matrix inversion** to reliably recover dark-mode or low-contrast barcodes. Extracts Virtual Payment Address (VPA) syntax, UPI payment parameter anomalies (`pa`, `pn`, `am`), embedded redirect shorteners, and obfuscated phishing URIs. Stage 2 triggers an intelligent vision fallback for artistic or logo-overlaid payment matrices using the multimodal `llama-3.2-11b-vision-preview` model on Groq.
5. **Vision & Synthetic Image Engine (`/api/investigate/image`):**
   ...
   - **Two-Stage Vision & Threat Inference:** Employs multimodal `llama-3.2-11b-vision-preview` in Stage 1 to process base64 image data without 400 Bad Request errors, followed by `qwen/qwen3.8-27b` in Stage 2 for deterministic threat analysis and structured JSON reporting.
...
- **Vision & Synthetic Media Engine (`Llama-3.2-11B-Vision-Preview` & `Qwen-3.8-27B`):** Multi-stage specialization where `llama-3.2-11b-vision-preview` ingests raw base64 RGB image tensors during Stage 1 visual analysis and Stage-2 QR fallback decodes, while `qwen/qwen3.8-27b` conducts pure-text forensic threat classification and IoC structuring. Fully asynchronous execution prevents thread-pool starvation.
