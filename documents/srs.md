...
### 1. Purpose
The TRINETRA platform serves as an AI-powered digital artifact forensics platform designed for security analysts and incident responders. Its primary purpose is to automate the extraction, parsing, and initial threat analysis of suspicious digital payloads (URLs, Emails, PDFs, QR Codes, and Images). By performing localized deterministic parsing in RAM prior to AI analysis, the system accelerates incident response times while mitigating payload exposure and zero-day execution risks.
...
- **Presentation Layer (Tactical Client):** A React 18 web application featuring an asynchronous state-machine UI and an interactive Cybernetic Third Eye HUD, a 7-stage investigation progression orchestrator, and interactive report visualization.
...
- **Extraction Layer (Local Core):** Dedicated Python engines utilizing `PyMuPDF` (`fitz`), `pytesseract`, `duckduckgo-search`, `zxing-cpp`, `OpenCV`, native `email` libraries, and brand typosquatting heuristics to strip binary noise and extract pure Indicators of Compromise (IoCs).
...
2. **Email Forensics:** Parse `.eml` and raw email structures in memory (bypassing multipart container wrappers) to analyze routing headers, resolve IP geolocations asynchronously via worker threadpools (`run_in_threadpool`), verify sender authentication (SPF/DKIM/DMARC alignment), evaluate urgency-based social engineering, inspect embedded tracking URLs, and pass all embedded/attached images simultaneously to the Vision LLM.
...
4. **QR Code (Quishing) Decoder:** Employ a hybrid decoding architecture combining Stage-1 local OpenCV/ZXing-CPP matrix reading (with bitwise-NOT dark-mode inversion) and Stage-2 Groq Vision AI fallback (`llama-3.2-90b-vision-instruct`) to decode stylized or logo-overlaid matrices (e.g., GPay/UPI overlays). Provide empathetic, non-technical threat evaluations while recognizing benign payment links (`upi://pay`).
5. **Vision & Image Engine:** Perform local OCR via threaded `pytesseract` and 2D FFT (Fast Fourier Transform) spectrum analysis in worker threadpools (`run_in_threadpool`) to eliminate event-loop blocking, execute live keyless DuckDuckGo OSINT queries with 15-word query truncation and a strict 5-second timeout wrapper, and leverage multimodal vision LLMs (`llama-3.2-90b-vision-instruct` for image parsing; `qwen/qwen3.8-27b` for threat reasoning) with an enhanced 4-Step Forensic Audit to detect AI-generated synthetic media, deepfake markers, and deceptive interface screenshots (including UI typography melting, non-hex hash IDs, and distorted panel geometry).
...
- **Dynamic AI Core Telemetry & Command Bridge:** The executive navbar must continuously track real-time client connectivity via `navigator.onLine` events, dynamically toggling between `ONLINE` (emerald glow) and `OFFLINE` (rose glow) status beacons.
- **Unified Triad of Forensic Terminal Modals:** The system must provide three architecturally unified, wide terminal inspection modals (`SYSTEM.ABOUT`, `REASONING.GROQ`, and `PRIVACY.SHIELD`). All modals must enforce high-opacity glassmorphism (`bg-black/90 backdrop-blur-md`), top-right `[ESC]` buttons with keyboard Escape listeners, and multi-colored semantic accent palettes across terminal sub-cards.
...
- **Environment-Aware Rate Limiting & Payload Restriction:** SlowAPI limits traffic to 6 requests/minute per IP (automatically disabled when ENVIRONMENT=development to prevent testing friction). A custom middleware enforces a hard 10MB upload limit to neutralize OOM risks and DoS attacks against cloud inference APIs.
