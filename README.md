...
- **Screenshot Vision OCR & Synthetic Media Engine**: Extracts text locally via `pytesseract` and evaluates visual streams using cryptographic C2PA manifests, EXIF metadata, Error Level Analysis (ELA) scores, and 2D Fast Fourier Transform (FFT) spectrum analysis to detect AI-generated synthetic artifacts and deepfake markers prior to LLM threat synthesis.
...
- **Extraction Libraries**: `PyMuPDF` (fitz), `zxing-cpp`, `OpenCV` (cv2), `pytesseract`, `c2pa-python`, `Pillow`, native Python `email` module
